  ...
  return (
    <footer className="bg-brand-black text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mt-12 pt-6 border-t border-gray-800 flex flex-col sm:flex-row justify-end gap-12 items-center text-sm text-gray-500">
          <p>&copy; {new Date().getFullYear()} M&T Printing Group. All rights reserved.</p>
          <div className="flex items-center gap-3 flex-wrap justify-end">
            {["3M", "Orafol", "Gerber", "Avery Dennison", "Tekton"].map((brand) => {
              const isTekton = brand === "Tekton";
              return (
                <div
                  key={brand}
                  className={`flex items-center justify-center px-2 rounded ${isTekton ? "h-10" : "h-6"}`}
                  style={{ backgroundColor: "rgba(255,255,255,0.05)" }}
                >
                  <img
                    src={`/Partners/${brand.replace(/ /g, "-")}.png`}
                    alt={brand}
                    className={`${isTekton ? "h-8" : "h-4"} object-contain opacity-60 hover:opacity-100 transition-opacity`}
                    onError={(e) => {
                      const parent = (e.target as HTMLImageElement).parentElement;
                      if (parent) parent.innerHTML = `<span class="text-[10px] font-bold text-gray-500 tracking-wide">${brand}</span>`;
                    }}
                  />
                </div>
              );
